...
import exceptions.ResultNotEqual;
...
public class CalculateProfitsGUI extends JFrame {
	...
	private final JButton btnApustuaEzabatu = new JButton(ResourceBundle.getBundle("Etiquetas").getString("CalculateProfits")); //$NON-NLS-1$ //$NON-NLS-2$
	...
	public CalculateProfitsGUI() {
		try {
			setTitle(ResourceBundle.getBundle("Etiquetas").getString("CalculateProfitsGUI"));
			jFind();
		} catch (Exception e) {
			e.printStackTrace();
		}
	...
	private void jFind() throws Exception {
			...
			public void actionPerformed(ActionEvent e) {		
				try {
					for(Kuota k: b.getKuota()) {
						if((k.getQuestion().getResult() == null) || k.getQuestion().getResult().equals("")) {
							throw new ResultNotEqual(ResourceBundle.getBundle("Etiquetas").getString("GalderaEguneratuGabe"));
						}else if(!k.getQuestion().getResult().equals(k.getResult())) {
							facade.removeBet(b);
							...
							throw new ResultNotEqual(ResourceBundle.getBundle("Etiquetas").getString("ApustuaEzIrabazi"));
						}
					}
					facade.calculateProfits(b);
					facade.removeBet(b);
					JOptionPane.showMessageDialog(contentPane, ResourceBundle.getBundle("Etiquetas").getString("ApustuaIrabazi"), "Info", JOptionPane.INFORMATION_MESSAGE);
					tableModelBets.removeRow(tableBets.getSelectedRow());
					tableModelBets.fireTableDataChanged();
					tableModelQuotas.setDataVector(null, columnNamesKuotak);
					tableModelQuotas.fireTableDataChanged();
				}catch(ResultNotEqual el) {
		...
		btnApustuaEzabatu.setBounds(62, 307, 177, 30);
